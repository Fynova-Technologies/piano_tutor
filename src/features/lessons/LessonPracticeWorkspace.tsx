...
/** Shared OSMD + BeatCursor + playback surface for official CDN lessons and AI recovery drills. */
export type LessonPracticeWorkspaceProps = {
  cdnFileName: string | null;
  externalXml: string | null;
  /** Increment when injecting new XML so OSMD remounts reliably. */
  xmlRenderKey: number;
  courseTitle: string;
  lessonSource: string;
  lessonId: string;
        ...
        setTimeout(() => {
          ...
          if (svgEl) {
            ...
            if (bbox.height > 0) {
              ...
              svgEl.setAttribute(
                "viewBox",
                `0 0 ${svgEl.getAttribute("width")} ${bbox.height + 20}`
              );
            }
          }
        }, 200);
...
function trackAndHighlightNote(midi: number) {
  ...
  if (isNoteStart) {
    ...
    if (isCorrect) {
      ...
      recordMistake("wrong_pitch");
      beatCursorRef.current.flashIncorrect(); // ← immediate color change
    }
  } else if (!exactMatch) {
    incorrectNotesRef.current += 1;
    recordMistake("timing_or_extra_note");
  ...
  setShowScorePopup={setShowScorePopup}
      ...
      <div
        style={{
          ...
          maxHeight: "min(420px, calc(100vh - 12rem))",
          overflowY: "auto",
