...
function LibraryPlayerContent() {
  ...
  // file param comes from the JSON, e.g. "/library/Arabesque_L._66_No._1_in_E_Major.mxl"
  // This is served directly from Next.js public folder
  const fileName = searchparams.get("file") || "";
  const source = searchparams.get("source") || "library";

  // The xml URL to load into OSMD — served from /public via Next.js static serving
  const xml = fileName;

  // ── Refs & State (unchanged from your implementation) ──────────────────────
  ...
  // ── OSMD init — loads xml from public folder ────────────────────────────────
  useEffect(() => {
    ...
    if (!xml) {
      console.error("No file param provided");
      return;
    }
    ...
    (async () => {
      try {
        // xml is e.g. "/library/Canon_in_D.mxl" — Next.js serves this from /public
        await osmd.load(xml);
    ...
    navigator.requestMIDIAccess({ sysex: false }).then((m) => {
      ...
      for (const input of m.inputs.values()) {
        ...
        input.onmidimessage = async (ev) => {
          ...
          if (isNoteOn && playModeRef.current) {
      ...
      >
        {(() => {
          ...
          return info ? (
            ...
            <div>
              {xml ? "Loading score…" : "⚠️ No file specified"}
            </div>
          );
        })()}
      </div>
