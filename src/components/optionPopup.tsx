...
export default function OptionPopup({ openDialogue, setOpenDialogue }: OptionPopupProps) {
  ...
  return (
    <div
  className="fixed inset-0 bg-[#5e5c5c82] overflow-y-auto overscroll-contain z-55"
              ...
              <React.Fragment key={optKey}>
                <div className="flex flex-col justify-between bg-gradient-to-r from-[#F2F2F2] to-[#EAEAEA] rounded-2xl p-3 md:p-4">
                  <div className="flex items-center justify-between border-b pb-2 gap-2">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                      <button
                        className="p-2 md:p-3 bg-[#F8C8C3] rounded-full text-xl flex items-center justify-center"
                        onClick={() =>
                      ...
                      <span className="w-8 text-center font-medium text-[18px] md:text-[20px] text-[#000000]">
                        {optValue}
                      </span>
                      <button
                        className="p-2 md:p-3 bg-[#2ECC71] rounded-full text-xl"
                        onClick={() =>
                          updateValue(optKey as keyof OptionSettingsData, 1)
                        }
                      >
                  ...
                  {optKey === "end_beat" && (
                    <button
                      ...
                      className="mt-3 md:mt-4 bg-[#D4AF37] w-full p-3 md:p-4 rounded-2xl flex gap-2 primary-color-text items-center justify-center text-[14px] md:text-[16px] cursor-pointer"
                    >
                ...
                {soundKey && icon && (
                  <div className="bg-gradient-to-r from-[#FFFFFFE5] via-[#D8D8D8] to-[#D8D8D8] rounded-2xl p-3 md:p-4 flex flex-col justify-between">
                    <div className="flex gap-3 items-center mb-2 md:mb-4">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <Slider.Root
                        className="relative flex items-center select-none w-full max-w-[300px] h-4"
                        min={0}
                        max={100}
                        step={1}
                        value={[volume]}
                      ...
                      <span className="text-black font-bold w-[20%] ml-2 md:ml-4">{volume}</span>
                    </div>
                  </div>
                )}
              </React.Fragment>
